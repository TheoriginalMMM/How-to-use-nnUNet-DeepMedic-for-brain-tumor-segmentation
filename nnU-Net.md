...
We very strongly recommend you install nnU-Net in a virtual environment. You will find [here](How-to-Setup-Python-Virtual-Environment.md) instructions to setup Python Virtual Environment
...
1) Install [PyTorch](https://pytorch.org/get-started/locally/) You need at least version 1.6. You can find the command to use to install **PyTorch** in the last ink depending on:
   * Your operating system.  
   * Which package you would like to use to complete this installation.   **To be competed**
   * Which version of CUDA you have.(you need to install CUDA if you want to use GPU).
...
[this](dataset_conversion.md) for information on how to convert datasets to be compatible with nnU-Net.
...
also see [here](dataset_conversion.md)), you can run this step with the following command:
...
running the cross-validation) are desired. See [FAQ](https://github.com/MIC-DKFZ/nnUNet/blob/master/documentation/common_questions.md) for more information.
...
You can then download models by specifying their task name. For the BraTS Brain Tumor Segmentation Challenge, 
...
these models has specific data requirements (Task001 runs on brain MRI scans, and  it requires those modalities as input : FLAIR,T1,T1 with contrast agent and T2).

If you would like to get the information and the requirements of a pre-trained model use the next command by replacing the task Id.
```bash
...
- run a training with the 3d full resolution U-Net on the Hippocampus dataset. See [here](training_example_Hippocampus.md).
- run inference with nnU-Net's pretrained models on the Prostate dataset. See [here](inference_example_Prostate.md).
